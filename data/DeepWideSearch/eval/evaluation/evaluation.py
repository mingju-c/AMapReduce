...
def evaluate_single_query(
    ...
    try:
        ...
        entity_llm_pass = "yes" in response.content.lower()
        entity_acc = 1.0 if entity_llm_pass else 0.0
    ...
    # If LLM said entity mismatch but any metric > 0, still treat entity as hit
    has_positive_metric = any(
        [
            score > 0,
            precision_by_row > 0,
            recall_by_row > 0,
            ...
            recall_by_item > 0,
            f1_by_item > 0,
            precision_by_unique_col > 0,
            recall_by_unique_col > 0,
            f1_by_unique_col > 0,
        ]
    )
    if not entity_llm_pass and has_positive_metric:
        entity_acc = 1.0
        msg = (msg + "\n" if msg else "") + "entity override: metrics > 0 despite llm=no"
