...
from .prompt import ExpmemoryPrompts
from mas.utils import load_json, write_json, random_divide_list
from mas.llm import LLMCallable, Message

def _format_pattern_block(pattern_info: Mapping | None) -> str:
    """Render pattern_info into a concise, human-readable hint block."""
    ...
    cols = tm.get("columns")
    task_matrix_summary = f"rows={tm.get('num_rows', '?')}, cols={tm.get('num_cols', '?')}"
    if cols and isinstance(cols, list):
        task_matrix_summary += f", column_names={', '.join(map(str, cols))}"

    schema_cols = schema.get("columns")
    schema_summary = f"schema_columns={', '.join(map(str, schema_cols))}" if schema_cols else "schema_columns=unknown"

    batching_summary = f"type={bs.get('type', '?')}, chunk_size={bs.get('chunk_size', '?')}"
    if bs.get("strategy_info"):
        batching_summary += f", strategy_info={bs.get('strategy_info')}"
    if bs.get("rationale"):
        batching_summary += f", rationale={bs.get('rationale')}"

    template_summary = f"template_complexity={tpl.get('complexity', 'unknown')}"
    if tpl.get("content"):
        template_summary += f", content_preview={str(tpl.get('content'))[:200]}"
...
@dataclass
class Expmemory(MASMemoryBase):
    """
    Expmemory: Tracing Hierarchical Memory for Multi-Agent Systems
    ...
    def __post_init__(self):
        ...
        self._start_insights_threshold: int = self.global_config.get('start_insights_threshold', 40)
        
        self._rounds_per_insights: int = self.global_config.get('rounds_per_insights', 20)
        
        self._insights_point_num: int = self.global_config.get('insights_point_num', 5)
        
        self._merge_insights_interval: int = self.global_config.get('merge_insights_interval', 40)
    ...
    def _get_hyperparams_dict(self) -> dict:
        return {
            ...
            'merge_insights_interval': self._merge_insights_interval,
            'working_dir': self.persist_dir
        }

    def _build_document_id(self, mas_message: MASMessage) -> str:
        """
        Build a stable identifier for each task/trial so repeated runs overwrite
        the same record instead of duplicating it in the vector store.
        """
        extra_fields: dict = getattr(mas_message, "extra_fields", {}) or {}
        instance_id = extra_fields.get('instance_id')
        trial_idx = extra_fields.get('trial_idx')
        call_stage = extra_fields.get('mapreduce_call_stage')

        parts: list[str] = []
        if instance_id:
            parts.append(str(instance_id))
        if trial_idx is not None:
            parts.append(f"trial-{trial_idx}")
        if call_stage:
            parts.append(str(call_stage))

        if not parts:

            base_text = (mas_message.task_main or "").strip()
            digest = hashlib.md5(base_text.encode('utf-8')).hexdigest()
            parts.append(digest)
    ...
    def add_memory(self, mas_message: MASMessage) -> None:
        ...
        if mas_message.label == True or mas_message.label == False:
            doc_id = self._build_document_id(mas_message)
            try:

                self.main_memory.delete(ids=[doc_id])
            except Exception:
                pass
            self.main_memory.add_documents([memory_doc], ids=[doc_id])
        else:
        ...
        if self.memory_size >= self._start_insights_threshold and self.memory_size % self._merge_insights_interval == 0:
            self.insights_layer.merge_insights() 

        self._index_done()

    def _format_pattern_info(self, pattern_info: Mapping | None) -> str:
        """Render pattern_info into a concise, human-readable hint block."""
        return _format_pattern_block(pattern_info)
    ...
    def _retrieve_memory_raw(
        ...
        coarse_success_k = successful_topk * 2 + 3 if successful_topk > 0 else 0
        coarse_failed_k = failed_topk * 2 + 3 if failed_topk > 0 else 0
        ...
        if len(true_tasks_doc) < successful_topk and coarse_success_k > 0:
            extra_docs = self.main_memory.similarity_search(
                query=query_task, k=coarse_success_k, filter={'label': True}
            )
            for doc in extra_docs:
                if doc not in true_tasks_doc:
                    true_tasks_doc.append(doc)

        if len(false_tasks_doc) < failed_topk and coarse_failed_k > 0:
            extra_docs = self.main_memory.similarity_search(
                query=query_task, k=coarse_failed_k, filter={'label': False}
            )
            for doc in extra_docs:
        ...
        def _dedup(messages: list[MASMessage]) -> list[MASMessage]:
            best_by_task: dict[str, tuple[MASMessage, float]] = {}
            order: list[str] = []
            for msg in messages:
                key = msg.task_main
                if not key or key == query_task:
                    continue
                extra = getattr(msg, "extra_fields", {}) or {}
                conf = extra.get("confidence_score")
                if not isinstance(conf, (int, float)):
                    conf = 0.0
                prev = best_by_task.get(key)
                if prev is None:
                    best_by_task[key] = (msg, conf)
                    order.append(key)
                else:
                    _, prev_conf = prev
                    if conf > prev_conf:
                        best_by_task[key] = (msg, conf)
            return [best_by_task[k][0] for k in order]

        true_task_messages = _dedup(true_task_messages)
        false_task_messages = _dedup(false_task_messages)
    ...
    def retrieve_memory(        
        ...
        **args
        ...
        for success_task in successful_task_trajectories:
            ...
            prompt: str = ExpmemoryPrompts.generative_task_user_prompt.format(
                trajectory=success_task.task_description + '\n' + success_task.task_trajectory,
                query_scenario=query_task
            )
            response: str = self.llm_model(messages=[Message('system', ExpmemoryPrompts.generative_task_system_prompt), 
                                                     Message('user', prompt)])
    ...
    def _extract_mas_message(self, mas_message: MASMessage) -> MASMessage:
        ...
        mas_message_copy.chain_of_states = StateChain()  
        mas_message_copy.task_trajectory = ""            
    ...
    def project_insights(self, raw_insights: list[str], role: str = None, task_traj: str = None) -> list[str]:
        ...
        if not task_traj:
            system_prompt = ExpmemoryPrompts.project_insights_system_prompt
            user_prompt: str = ExpmemoryPrompts.project_insights_user_prompt.format(
                role=role,
                insights=raw_insights_str
            )
        else:

            system_prompt = ExpmemoryPrompts.project_insights_with_traj_system_prompt
            user_prompt: str = ExpmemoryPrompts.project_insights_with_traj_user_prompt.format(
                role=role,
                insights=raw_insights_str,
...
class TaskLayer:    
    ...
    def cluster_tasks(self) -> None:
        ...
        try:
            ...
            print(f"FINCH clustering failed: {err}")
            try:
                legacy_result = FINCH(X, distance="cosine", verbose=False)
            except Exception as legacy_err:
                print(f"FINCH legacy clustering failed: {legacy_err}")
            else:
                clusters = legacy_result[0] if isinstance(legacy_result, tuple) else legacy_result
                if isinstance(clusters, np.ndarray):
                    labels = clusters[:, -1] if clusters.ndim > 1 else clusters

        if labels is None or len(labels) != len(valid_nodes):
            print(f"[debug] labels fallback: labels={None if labels is None else len(labels)}, valid_nodes={len(valid_nodes)}")
...
class InsightsManager:      
    ...
    @staticmethod
    def _format_pattern_info(pattern_info: Mapping | None) -> str:
        return _format_pattern_block(pattern_info)
        ...
        for label, related_rules in merged_label_rules.items():
            ...
            for rule in related_rules:
                insight: dict = {
                    'rule': rule,
                    'score': 2,          
                    'correlation_tasks': list(related_task_mains),
                }
    ...
    def _merge_rules(self, rules: list[str]) -> list[str]:
        ...
        for i in range(0, len(rules), batch_size):
            ...
            user_prompt = ExpmemoryPrompts.merge_rules_user_prompt.format(
                current_rules='\n'.join(batch),
                limited_number = max(1, len(batch) // 3)
            )
            messages = [Message('system', ExpmemoryPrompts.merge_rules_system_prompt),
                        Message('user', user_prompt)]
    ...
    def _retrieve_memory(       
        ...
        raw_success_topk = successful_topk * 4 + 3 if successful_topk > 0 else 0
        raw_failed_topk = failed_topk * 4 + 3 if failed_topk > 0 else 0

        if raw_success_topk > 0:
            true_tasks_doc = self.task_storage.similarity_search_with_score(
                query=query_task, k=raw_success_topk, filter={'label': True}
            )
        if raw_failed_topk > 0:
            false_tasks_doc = self.task_storage.similarity_search_with_score(
                query=query_task, k=raw_failed_topk, filter={'label': False}
            )

        def _dedup_docs_by_task(
            candidates: list[tuple[Document, float]],
            topk: int,
        ) -> list[tuple[Document, float]]:
            """
            Keep the highest-confidence trial per task_main, then rank by similarity
            and return the top-k distinct tasks.
            """
            best_by_task: dict[str, tuple[Document, float, float]] = {}
            for doc, distance in candidates:

                sim = 1 - distance
                meta = getattr(doc, "metadata", {}) or {}
                task_main = meta.get("task_main")
                if not task_main or task_main == query_task:
                    continue
                extra_raw = meta.get("extra_fields") or "{}"
                try:
                ...
                if not isinstance(conf, (int, float)):
                    conf = 0.0
                prev = best_by_task.get(task_main)
                if prev is None:
                    best_by_task[task_main] = (doc, sim, conf)
                else:
                    _, _prev_sim, prev_conf = prev
                    if conf > prev_conf:
                        best_by_task[task_main] = (doc, sim, conf)
            ranked = sorted(
                best_by_task.values(),
            ...
            return [(doc, sim) for (doc, sim, _conf) in selected]

        true_tasks_doc = _dedup_docs_by_task(true_tasks_doc, successful_topk)
        false_tasks_doc = _dedup_docs_by_task(false_tasks_doc, failed_topk)

        true_task_messages: list[MASMessage] = []
        false_task_messages: list[MASMessage] = []
        for doc, _sim in true_tasks_doc:
            meta_data: dict = doc.metadata
            mas_message: MASMessage = MASMessage.from_dict(meta_data)
            true_task_messages.append(mas_message)

        for doc, _sim in false_tasks_doc:
            meta_data: dict = doc.metadata
            mas_message: MASMessage = MASMessage.from_dict(meta_data)
            false_task_messages.append(mas_message)
    ...
    def _find_related_insights(
        ...
        for idx, rule in enumerate(self.insights_memory):
            score: int = sum(task in rule.get('correlation_tasks', []) for task in task_mains)
            if score >= threshold:
    ...
    def _finetune_insights(
        ...
        suffix: str = ExpmemoryPrompts.finetune_insights_suffix['full'] if len(self.insights_memory) > MAX_RULE_THRESHOLD \
                      else ExpmemoryPrompts.finetune_insights_suffix['not_full']
    ...
    def _build_comparative_prompts(self, true_traj: MASMessage, false_traj: MASMessage, insights: list[dict]) -> list[Message]:
        ...
        success_pattern = self._format_pattern_info(true_traj.get_extra_field('pattern_info'))
        failed_pattern = self._format_pattern_info(false_traj.get_extra_field('pattern_info'))

        prompt = ExpmemoryPrompts.critique_compare_rules_user_prompt.format(
            task1_pattern=success_pattern,
            task2_pattern=failed_pattern,
            existing_rules=rule_text
        )

        return [Message(role='system', content= ExpmemoryPrompts.critique_compare_rules_system_prompt), Message(role='user', content=prompt)] 
    ...
    def _build_success_prompts(
        ...
        for i, task in enumerate(success_trajectories):
            pattern = self._format_pattern_info(task.get_extra_field('pattern_info'))
            history.append(f'Case {i+1}: {task.task_description}\n{pattern}')
        prompt = ExpmemoryPrompts.critique_success_rules_user_prompt.format(
            success_history='\n'.join(history),
            existing_rules=rule_text
        )

        return [Message(role='system', content=ExpmemoryPrompts.critique_success_rules_system_prompt), Message(role='user', content=prompt)]
        ...
        for op in ['REMOVE', 'AGREE', 'EDIT', 'ADD']: 
            for i in range(len(operations)):
                ...
                if operation_type == 'REMOVE': 
                    ...
                    rule_data: dict = self.insights_memory[rule_index]
                    rule_data['score'] += 1
                    rule_data['correlation_tasks'] = list(set(rule_data['correlation_tasks'] + relative_tasks))
                    ...
                    meta_data: dict = {
                        ...
                        'score': 2,         
                        'correlation_tasks': list(relative_tasks),
                    }
