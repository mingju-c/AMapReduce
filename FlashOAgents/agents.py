...
class ToolCallingAgent(MultiStepAgent):
    ...
    def __init__(
        ...
        try:
            filename = prompt_filename or "toolcalling_agent.yaml"
            prompt_path = importlib.resources.files(f"FlashOAgents.prompts.{prompts_type}").joinpath(filename)
            self.prompt_templates = prompt_templates or yaml.safe_load(prompt_path.read_text(encoding="utf-8"))
        except FileNotFoundError:
            raise AgentError(f"No prompt file：{prompts_type}/{filename}")
    ...
    def step(self, memory_step: ActionStep, memory_messages=None) -> Union[None, Any]:
        ...
        try:
            ...
            flat_tool_calls: List[Dict[str, Any]] = []

            def _collect_tool_calls(item: Any) -> None:
                if isinstance(item, dict):
                    tool_name = (item.get("name") or "").strip()
                    if tool_name:
                        flat_tool_calls.append(item)
                    else:
                        try:
                            self.logger.log(
                                f"Dropping malformed tool payload (missing name): {item}",
                                level=LogLevel.WARNING,
                            )
                        except Exception:
                            logger.warning("Dropping malformed tool payload (missing name): %s", item)
                elif isinstance(item, list):
                    for sub_item in item:
                        _collect_tool_calls(sub_item)
                else:
                    try:
                        self.logger.log(
                            f"Ignoring malformed tool call entry of type {type(item).__name__}: {item}",
                            level=LogLevel.WARNING,
                        )
                    except Exception:
                        logger.warning(
                            "Ignoring malformed tool call entry of type %s: %s",
                            type(item).__name__,
                            item,
                        )

            _collect_tool_calls(tool_calls_list)
            tool_calls_list = flat_tool_calls
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:   
                ...
                for idx, tool_call in enumerate(tool_calls_list):
                    ...
                    if not tool_name:
                        try:
                            self.logger.log(
                                f"Skipping malformed tool call without name: {tool_call}",
                                level=LogLevel.WARNING,
                            )
                        except Exception:
                            logger.warning("Skipping malformed tool call without name: %s", tool_call)
                        continue
                    ...
                    if tool_name == "mapreducetool":
                        call_signature = json.dumps(tool_arguments, ensure_ascii=False, sort_keys=True)
                        seen_signatures = self.state.setdefault("_mapreduce_calls", [])
                        mapreduce_count = self.state.setdefault("_mapreduce_call_count", 0)
                        
                        # Check if this is a duplicate call with same arguments
                        if call_signature in seen_signatures:
                            warning_msg = (
                                "mapreducetool has already been executed with the same arguments. "
                                "Adjust the template or task_matrix when attempting a follow-up fill."
                            )
                            try:
                                self.logger.log(warning_msg, level=LogLevel.WARNING)
                            except Exception:
                                logger.warning(warning_msg)
                            observations.append(warning_msg)
                            continue
                        
                        # Enforce maximum 2 calls per task
                        if mapreduce_count >= 4:
                            warning_msg = (
                                f"mapreducetool has already been called {mapreduce_count} times. "
                                "Maximum 4 calls allowed per task. Please call final_answer with available results instead."
                            )
                            try:
                                self.logger.log(warning_msg, level=LogLevel.WARNING)
                            except Exception:
                        ...
                        seen_signatures.append(call_signature)
                        self.state["_mapreduce_call_count"] = mapreduce_count + 1
...
from .mapreducetool import MapReduceTool, mapreducetool
