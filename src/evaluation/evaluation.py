...
def evaluate_single_query(
    ...
    try:
        # pre check
        required_columns = json.loads(query.evaluation)["required"]
        unique_columns = json.loads(query.evaluation)["unique_columns"]
        ...
        # evaluation
            ...
            for col in unique_columns:
                item = json.loads(query.evaluation)["eval_pipeline"].get(col, None)
                if item is None:
                    continue
            ...
            for col, item in json.loads(query.evaluation)["eval_pipeline"].items():
            ...
            for col in required_columns:
                ...
                item = json.loads(query.evaluation)["eval_pipeline"][col]
                metric_func_name_list = item.get("metric", [])
                ...
                for metric_func_name in metric_func_name_list:
                    ...
                    # Convert Series of tuples to DataFrame
                    # Handle empty case: if df_inner is empty, metric_info_series will be empty
                    if len(metric_info_series) == 0:
                        # Empty Series case: create empty columns with proper dtype
                        df_inner_score[f"{col}_{metric_func_name}"] = pd.Series(dtype=float, index=df_inner.index)
                        df_inner_msg[f"{col}_{metric_func_name}_eval_msg"] = pd.Series(dtype=str, index=df_inner.index)
                    else:
                        # Normal case: metric_call returns (score, msg) tuple, so metric_info_df will have columns 0 and 1
                        metric_info_df = metric_info_series.apply(pd.Series)
                        df_inner_score[f"{col}_{metric_func_name}"] = metric_info_df[0]
                        df_inner_msg[f"{col}_{metric_func_name}_eval_msg"] = metric_info_df[1]
                    logger.debug(
