...
class WideSearchDataLoader:
    ...
    def load_data(self, data_path: str, answer_root: str):
        ...
        for item in data:
            ...
            item["answer"] = self.load_answer(
                answer_path, json.loads(item["evaluation"])["required"]
            )
    ...
    def get_instance_val_list(self):
...
class WideSearchResponse:
    ...
    def extract_dataframe(self) -> pd.DataFrame | None:
        ...
        if markdown_str:
            if markdown_str[0].startswith("{|") or markdown_str[0].startswith("{ |"):
                markdown_str[0] = "{\n|" + markdown_str[0][2:]
...
class WideSearchResponseLoader:
    ...
    def load_response(response_path: str) -> list[WideSearchResponse]:
        ...
        response_list = pd.read_json(response_path, lines=True).to_dict(orient="records")
        
        new_response_list = []
        for item in response_list:
            
            mapped_item = {
                "instance_id": item["task_id"],         # task_id -> instance_id
                "response": item["prediction"],         # prediction -> response
                "messages": item["intermediate_steps"], # intermediate_steps -> messages
                "trial_idx": item.get("trial_idx")     # trial_idx -> trial_idx (optional, maybe missing)
            }
            
            
            new_response_list.append(WideSearchResponse(**mapped_item))
